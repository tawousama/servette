...
class WebsiteForm(form.WebsiteForm):
    ...
    @http.route('/servette/request_info',  type='json', auth='public', csrf=False)
    def requestinfo(self, **data):
        new_parnter =request.env['res.partner'].sudo().create({
                'name': data['name']})
        request.env['crm.lead'].sudo().create(
            {
                'name': 'Opportunité de '+ data['name'],
                'partner_id': new_parnter.id,
                'phone': data['phone'],
                'email_from': data['email'],
                'description': '<strong>Question: </strong>' + data['question'] + '<br />' + '<strong>Choix de contact: </strong> '+ data['choix_contact'],
                'produits': [(4, int(data['product_id']))] })
        
        return {
            'success': 'envoi de demande info',
    ...
    def extract_data(self, model, values):
        ...
        for field_name, field_value in values.items():
            ...
            # If it's a known field
                ...
                if dest_model._name == 'crm.lead' and field_name == 'name':
                    data['record']['name'] += " " + str(values['last_name'])

            # If it's a custom field
            elif field_name != 'context':
                if dest_model._name == 'crm.lead' and field_name == 'products':
                    lines = request.env['sale.order.line'].search([('order_id', '=', int(values['products']))])
                    if lines:

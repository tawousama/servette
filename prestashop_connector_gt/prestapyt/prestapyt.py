...
class PrestaShopWebService(object):
    ...
    def _execute(self, url, method, data=None, files=None, add_headers=None):
        ...
        if not files:
            ...
            r = self.client.request(method, url, files=files,
                                    headers={'User-agent': 'Prestapi: Python Prestashop Library'})
    ...
    def add_with_url(self, url, content, img_filename=None):
        ...
        if not img_filename:
            ...
            # img_binary = content
            img_binary = base64.b64decode(content)
    ...
    def encode_multipart_formdata(self, files):
        print('\niles+++++encode_multipart_formdata+++++++', files)
...
#
#     def encode_multipart_formdata(self, files):
#         print('\nfiles+++++encode_multipart_formdata+++++++', files)
#         """Encode files to an http multipart/form-data.
#
#         :param files: a sequence of (type, filename, value)
#             elements for data to be uploaded as files.

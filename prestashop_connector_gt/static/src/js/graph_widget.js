odoo.define('gt_graph_widget.graph', function (require) {
    "use strict";
    ...
    var GtDashboardGraph = AbstractField.extend({
        className: "gt_dashboard_graph",
        events: {
        ...
        _renderInDOM: function () {
            ...
            this.$el.empty();
            if(this.graph_data){
                var dashboard = $(QWeb.render('gt_graph_dashboard',{widget: this}))
                this.$el.append(dashboard);
                this.$el.find('.gt_graph').append(this.$canvas);
            } else {
    ...
    fieldRegistry.add('gt_dashboard_graph', GtDashboardGraph);
    return {
        GtDashboardGraph: GtDashboardGraph
    };
});